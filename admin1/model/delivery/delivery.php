<?php
class ModelDeliveryDelivery extends Model {
	...
	public function getDelivery($delivery_id) {
		$query = $this->db->query("SELECT * from " . DB_PREFIX . "delivery WHERE delivery_id = 'delivery_id=" . (int)$delivery_id  . "'");

		return $query->row;
	}

	public function getDeliverys($data = array()) {
		$sql = "SELECT * FROM " . DB_PREFIX . "delivery";

		if (!empty($data['filter_name'])) {
			$sql .= " WHERE fname LIKE '" . $this->db->escape($data['filter_name']) . "%'";
		}
