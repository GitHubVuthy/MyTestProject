...
class ModelCatalogInfoCategory extends Model {
	public function addCategory($data) {
		...
		$info_category_id = $this->db->getLastId();

		if (isset($data['image'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "info_category SET image = '" . $this->db->escape($data['image']) . "' WHERE info_category_id = '" . (int)$info_category_id . "'");
		}

		foreach ($data['category_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "info_category_description SET info_category_id = '" . (int)$info_category_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', description = '" . $this->db->escape($value['description']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "'");
		}

		// MySQL Hierarchical Data Closure Table Pattern
		$level = 0;

		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "info_category_path` WHERE info_category_id = '" . (int)$data['parent_id'] . "' ORDER BY `level` ASC");

		foreach ($query->rows as $result) {
			$this->db->query("INSERT INTO `" . DB_PREFIX . "info_category_path` SET `info_category_id` = '" . (int)$info_category_id . "', `path_id` = '" . (int)$result['path_id'] . "', `level` = '" . (int)$level . "'");

			$level++;
		}

		$this->db->query("INSERT INTO `" . DB_PREFIX . "info_category_path` SET `info_category_id` = '" . (int)$info_category_id . "', `path_id` = '" . (int)$info_category_id . "', `level` = '" . (int)$level . "'");

		if (isset($data['category_filter'])) {
			foreach ($data['category_filter'] as $filter_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "info_category_filter SET info_category_id = '" . (int)$info_category_id . "', filter_id = '" . (int)$filter_id . "'");
			}
		}
		...
		if (isset($data['category_layout'])) {
			foreach ($data['category_layout'] as $store_id => $layout_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "info_category_to_layout SET category_id = '" . (int)$category_id . "', store_id = '" . (int)$store_id . "', layout_id = '" . (int)$layout_id . "'");
			}
		}
	...
	public function editCategory($category_id, $data) {
		...
		if (isset($data['image'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "info_category SET image = '" . $this->db->escape($data['image']) . "' WHERE category_id = '" . (int)$category_id . "'");
		}
	...
	public function deleteCategory($category_id) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "info_category_to_store WHERE category_id = '" . (int)$category_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "info_category_to_layout WHERE category_id = '" . (int)$category_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "info_product_to_category WHERE category_id = '" . (int)$category_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "url_alias WHERE query = 'category_id=" . (int)$category_id . "'");
	...
	public function getCategory($category_id) {
		$query = $this->db->query("SELECT DISTINCT *, (SELECT GROUP_CONCAT(cd1.name ORDER BY level SEPARATOR '&nbsp;&nbsp;&gt;&nbsp;&nbsp;') FROM " . DB_PREFIX . "info_category_path cp LEFT JOIN " . DB_PREFIX . "info_category_description cd1 ON (cp.path_id
		 = cd1.category_id AND cp.info_category_id != cp.path_id) WHERE cp.info_category_id = c.category_id AND cd1.language_id = '" . (int)$this->config->get('config_language_id') . "' GROUP BY cp.info_category_id) AS path, (SELECT DISTINCT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'category_id=" . (int)$category_id . "') AS keyword FROM " . DB_PREFIX . "info_category c LEFT JOIN " . DB_PREFIX . "info_category_description cd2 ON (c.category_id = cd2.category_id) WHERE c.category_id = '" . (int)$category_id . "' AND cd2.language_id = '" . (int)$this->config->get('config_language_id') . "'");

		return $query->row;
	}

	public function getCategories($data = array()) {
		$sql = "SELECT cp.info_category_id AS info_category_id, GROUP_CONCAT(cd1.name ORDER BY cp.level SEPARATOR '&nbsp;&nbsp;&gt;&nbsp;&nbsp;') AS name, c1.parent_id, c1.sort_order FROM " . DB_PREFIX . "info_category_path cp LEFT JOIN " . DB_PREFIX . "category c1 ON (cp.info_category_id = c1.category_id) LEFT JOIN " . DB_PREFIX . "info_category c2 ON (cp.path_id = c2.category_id) LEFT JOIN " . DB_PREFIX . "info_category_description cd1 ON (cp.path_id = cd1.category_id) LEFT JOIN " . DB_PREFIX . "info_category_description cd2 ON (cp.info_category_id = cd2.category_id) WHERE cd1.language_id = '" . (int)$this->config->get('config_language_id') . "' AND cd2.language_id = '" . (int)$this->config->get('config_language_id') . "'";

		if (!empty($data['filter_name'])) {
			$sql .= " AND cd2.name LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
		}

		$sql .= " GROUP BY cp.info_category_id";
	...
	public function getCategoryDescriptions($category_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "info_category_description WHERE info_category_id = '" . (int)$info_category_id . "'");
	...
	public function getCategoryPath($category_id) {
		$query = $this->db->query("SELECT info_category_id, path_id, level FROM " . DB_PREFIX . "info_category_path WHERE info_category_id = '" . (int)$info_category_id . "'");

		return $query->rows;
	}
	
	public function getCategoryFilters($info_category_id) {
		$category_filter_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "info_category_filter WHERE info_category_id = '" . (int)$info_category_id . "'");
	...
	public function getCategoryStores($info_category_id) {
		$category_store_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "info_category_to_store WHERE info_category_id = '" . (int)$info_category_id . "'");
	...
	public function getCategoryLayouts($info_category_id) {
		$category_layout_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "info_category_to_layout WHERE info_category_id = '" . (int)$info_category_id . "'");
