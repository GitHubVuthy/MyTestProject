...
class ModelCustomerCustomer extends Model {
	...
	public function getCustomers($data = array()) {
		...
		if (!empty($data['filter_name'])) {
			$implode[] = "CONCAT(c.firstname, ' ', c.lastname) OR telephone LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
		}
...
public function getExportCustomers($data = array()) {
		/*$sql = "SELECT *, CONCAT(c.firstname, ' ', c.lastname) AS name, cgd.name AS customer_group FROM " . DB_PREFIX . "customer c LEFT JOIN " . DB_PREFIX . "customer_group_description cgd ON (c.customer_group_id = cgd.customer_group_id) WHERE cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";*/
		$sql="SELECT c.customer_id, c.date_added,c.telephone,concat(c.firstname, ' ',c.lastname) as name ,a.address_1, a.company, cgd.name as customer_group FROM `" . DB_PREFIX . "customer` c INNER JOIN " . DB_PREFIX . "address a on (c.customer_id=a.customer_id) INNER JOIN " . DB_PREFIX . "customer_group_description cgd on (c.customer_group_id=cgd.customer_group_id) where cgd.language_id = '" . (int)$this->config->get('config_language_id') . "' 
		";
