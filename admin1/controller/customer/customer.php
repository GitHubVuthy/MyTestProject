...
public function export() {
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->get['page'])) {
				...
				//I suppose it redirect to page after done
			$this->response->redirect($this->url->link('customer/customer', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	public function unlock() {
		...
		if (isset($this->request->get['telephone']) && $this->validateUnlock()) {
			$this->model_customer_customer->deleteLoginAttempts($this->request->get['telephone']);
		...
		protected function getCustomer_export() {
		if (isset($this->request->get['filter_name'])) {
	...
	/*	if (isset($this->request->get['filter_status'])) {
		...
		$results = $this->model_customer_customer->getExportCustomers($filter_data);
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$login_info = $this->model_customer_customer->getTotalLoginAttempts($result['telephone']);

			if ($login_info && $login_info['total'] >= $this->config->get('config_login_attempts')) {
				$unlock = $this->url->link('customer/customer/unlock', 'token=' . $this->session->data['token'] . '&telephone=' . $result['telephone'] . $url, true);
			} else {
				$unlock = '';
			}
