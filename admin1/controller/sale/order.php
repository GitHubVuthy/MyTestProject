...
public function export() {
		...
		$this->getOrder_export();
	}

protected function getOrder_export() {
...
*/
	...
	/*	$data['sort_order'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&sort=o.order_id' . $url, true);
		...
		$this->response->setOutput($this->load->view('sale/order_export', $data));
	}
	...
	public function getForm() {
		...
		if (!empty($order_info)) {
			...
			/*$data['delivery_id'] = $order_info['delivery'];*/
...
// Stores
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				$data['orders'][] = array(
					...
					'shipping_address'  => $order_info['address_place'],
	...
	public function shipping() {
		...
		if (isste($this->request->post['selected'])) {
			$orders = $this->request->post['selected'];
		} elseif (isset($this->request->get['order_id'])) {
			$orders[] = $this->request->get['order_id'];
		}
