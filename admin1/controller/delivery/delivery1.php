<?php
class ControllerDeliveryDelivery1 extends Controller {
	private $error = array();
	public function index() {
		$this->load->language('delivery/delivery1');
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $delivery_id) {
				$this->model_delivery_delivery1->deleteDelivery($delivery_id);
			}
			...
			$this->response->redirect($this->url->link('delivery/delivery1', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function validateDelete() {
		if (!$this->user->hasPermission('modify', 'delivery/delivery1')) {
		...
		foreach ($this->request->post['selected'] as $delivery_id) {
			$delivery_total = $this->model_delivery_delivery1->getTotalDeliveryByDeliveryId($delivery_id);
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_delivery_delivery1->editDelivery($this->request->get['delivery_id'], $this->request->post);
...
public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_delivery_delivery1->addDelivery($this->request->post);
	...
	protected function validateForm() {
		...
		if ((utf8_strlen($this->request->post['phone']) < 9) || (utf8_strlen($this->request->post['fname']) > 10)) {
	...
	protected function getForm() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('delivery/delivery1', 'token=' . $this->session->data['token'] . $url, true)
		);

		if (!isset($this->request->get['delivery_id'])) {
			$data['action'] = $this->url->link('delivery/delivery1/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('delivery/delivery1/edit', 'token=' . $this->session->data['token'] . '&delivery_id=' . $this->request->get['delivery_id'] . $url, true);
		}
		                                                                                                                                                                                           

		$data['cancel'] = $this->url->link('delivery/delivery1', 'token=' . $this->session->data['token'] . $url, true);

		if (isset($this->request->get['delivery_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$delivery_info = $this->model_delivery_delivery1->getDelivery($this->request->get['delivery_id']);
		}
		...
		} elseif (!empty($delivery_info) && is_file(DIR_IMAGE . $delivery_info['delivery_image'])) {
			$data['thumb'] = $this->model_tool_image->resize($delivery_info['delivery_image'], 100, 100);
		} else {
		...
		$this->response->setOutput($this->load->view('delivery/delivery_form1', $data));
	}
	...
	protected function getList() {
		...
		$data['add'] = $this->url->link('delivery/delivery1/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('delivery/delivery1/delete', 'token=' . $this->session->data['token'] . $url, true);
...
/*here*/
		...
		/*for Pagination*/ $delivery_total = $this->model_delivery_delivery1->getTotalDeliverys();
		...
		foreach ($results as $result) {
			$data['deliverys'][] = array(
				...
				'edit'            => $this->url->link('delivery/delivery1/edit', 'token=' . $this->session->data['token'] . '&delivery_id=' . $result['delivery_id'] . $url, true),
			);
		}
		...
		$data['sort_fname'] = $this->url->link('delivery/delivery1', 'token=' . $this->session->data['token'] . '&sort=fname' . $url, true);
		$data['sort_sort_order'] = $this->url->link('delivery/delivery1', 'token=' . $this->session->data['token'] . '&sort=sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('delivery/delivery1', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('delivery/delivery_list1', $data));
	}
