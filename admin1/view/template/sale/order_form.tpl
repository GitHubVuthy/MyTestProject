          ...
          <div class="tab-content">
           ...
           <!-- 
                 <div class="form-group">
                  <label class="col-sm-2 control-label" for="input-delivery"><?php echo "Choose Delivery"; ?></label>
                  <div class="col-sm-10">
                  <select name="delivery_id" id="input-delivery" class="form-control">
                      <?php foreach ($deliveries as $delivery) { ?>
                      <?php if ($delivery['delivery_id'] == $delivery_id) { ?>
                      <option value="<?php echo $delivery['delivery_id']; ?>" selected="selected"><?php echo  $delivery['fname'] ; ?></option>
                      <?php } else { ?>
                      <option value="<?php echo $delivery['delivery_id']; ?>"><?php echo $delivery['fname'] ; ?></option>
                      <?php } ?>
                      <?php } ?>
                  </select>
             ...
             <div class="form-group required">
              ...
              <div class="form-group ">
                ...
                <div class="col-sm-10">
                  <input type="text" name="email" value="imvuthy11@gmail.com" id="input-email" class="form-control" placeholder="email"/>
                </div>
              </div>
			  -->
              <div class="form-group required">
               ...
               <div class="form-group">
                  ...
                  <div class="col-sm-10">
                    <textarea name="comment" rows="2" id="input-comment" class="form-control"><?php echo $comment; ?></textarea>
                  </div>
                </div>
            ...
            <div class="tab-pane" id="tab-cart">
              ...
              <div class="row">
                ...
                <div class="col-sm-6 text-right">
                  ...
                  <button type="button" id="button-save" class="btn btn-success btn-lg"><i class="fa fa-check-circle"></i> <?php echo $button_save; ?></button>
                </div>
              </div>
            </div>
...
$('input[name=\'customer\']').autocomplete({
	...
	'select': function(item) {
		...
		$('#tab-customer select option').not($('#tab-customer select[name=\'store_id\'] option, #tab-customer select[name=\'delivery_id\'] option, #tab-customer input[name=\'email\'],  #tab-customer select[name=\'currency\'] option')).removeAttr('selected');
		...
		for (i in  item['address']) {
			html += '<option value="' + item['address'][i]['address_id'] + '">' + item['address'][i]['address_1'] + '</option>';
		}

	/*	$('select[name=\'payment_add\']').html(html);*/
		$('select[name=\'shipping_address\']').html(html);

		/*$('select[name=\'payment_addres\']').trigger('change');*/
...
//Customer Testing
/*$('input[name=\'customer'\']).automcomplete({
  'source': function(request, response) {
        $.ajax({
          url:'index.php?route=customer/customer/automcomplete&token=<?php echo $token; ?>&filter_name=' + encodeURIComponent(request),
          dataType: 'json';
          success: function(json){
            json.unshift({
              customer_id: '0',
  ...
  'select': function(item){
    //Reset all custom fields
    $('#tab-customer input [type=\'\ text\'],  #tab-customer textarea').not('#tab-customer input [name=\'customer\'], #tab-customer
...
// Payment Address
/*$('select[name=\'pament_address\']').on('change', function() {
	$.ajax({
...
$('#button-save').on('click', function() {
	...
	$.ajax({
		url: url,
		type: 'post',
		data: $('select[name=\'payment_method\'] option:selected,  select[name=\'shipping_method\'] option:selected,  #tab-total select[name=\'order_status_id\'],#tab-customer select[name=\'delivery_id\'], #tab-customer select[name=\'shipping_id\'],#tab-customer select[name=\'order_time\'], #tab-total select, #tab-total textarea[name=\'comment\'], #tab-total input[name=\'affiliate_id\']'),
		dataType: 'json',
		crossDomain: true,
		beforeSend: function() {
