...
class ModelCatalogInfoCategory extends Model {
	...
	public function getInfoCategories($parent_id = 0) {
		$query = $this->db->query("SELECT *,c.category_id FROM " . DB_PREFIX . "info_category c LEFT JOIN " . DB_PREFIX . "info_category_description cd ON (c.category_id = cd.category_id) LEFT JOIN " . DB_PREFIX . "info_category_to_store c2s ON (c.category_id = c2s.category_id) WHERE c.parent_id = '" . (int)$parent_id . "' AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND c2s.store_id = '" . (int)$this->config->get('config_store_id') . "'  AND c.status = '1' ORDER BY c.sort_order, LCASE(cd.name)");

		return $query->rows;
	}

	public function getInfoCategoryFilters($category_id) {
		$implode = array();
	...
	public function getInfoCategoryLayoutId($category_id) {
	...
	public function getInfoTotalCategoriesByCategoryId($parent_id = 0) {
	...
	public function getTotalInfoCategories($parent_id = 0) {
...
public function getTotalInfoCategory($category_id) {
		$query = $this->db->query("SELECT COUNT(category_id) as total FROM ". DB_PREFIX ."info_category where parent_id='".(int)$category_id."' ");
