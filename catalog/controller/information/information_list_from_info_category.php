<?php
class ControllerInformationInformationListFromInfoCategory extends Controller {
	public function index() {
		...
		if ($info_category_info) {
			...
			$data['breadcrumbs'][] = array(
				'text' => $info_category_info['name'],
				'href' => $this->url->link('information/information_list_from_info_category', 'path=' . $this->request->get['path'] .'&category_id=' . $this->request->get['category_id'])
			);
			...
			$pagination->url = $this->url->link('information/information_list_from_info_category', 'path=' . $this->request->get['path'] . '&category_id='.$category_id. $url . '&page={page}');
			...
			if ($page == 1) {
			    $this->document->addLink($this->url->link('information/information_list_from_info_category', 'path=' . $info_category_info['category_id'], true), 'canonical');
			} elseif ($page == 2) {
			    $this->document->addLink($this->url->link('information/information_list_from_info_category', 'path=' . $info_category_info['category_id'], true), 'prev');
			} else {
			    $this->document->addLink($this->url->link('information/information_list_from_info_category', 'path=' . $info_category_info['category_id'] . '&page='. ($page - 1), true), 'prev');
			}

			if ($limit && ceil($information_total / $limit) > $page) {
			    $this->document->addLink($this->url->link('information/information_list_from_info_category', 'path=' . $info_category_info['category_id'] . '&page='. ($page + 1), true), 'next');
			}
			...
			$this->response->setOutput($this->load->view('information/information_list_from_info_category', $data));
		} else {
			$url = '';
