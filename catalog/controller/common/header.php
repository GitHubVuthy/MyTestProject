...
class ControllerCommonHeader extends Controller {
	public function index() {
		...
		if (is_file(DIR_IMAGE . $this->config->get('config_logo'))) {
			...
			$data['bheader'] = $server . 'image/catalog/whitegarlic-opencart-logo.png';
		} else {
			$data['logo'] = '';
		...
		$data['text_weekly_food_menu'] = $this->language->get('text_weekly_food_menu');
		//Food and Health
		...
		$data['next_week_menu'] = $this->url->link('product/next_week_menu');
		...
		$data['text_next_week_menu'] =  $this->language->get('text_next_week_menu');
		// Wishlist
...
// Service link
		$data['daily_food']=$this->url->link('information/information', 'information_id=54');	
...
// Cooking Learning
		$data['by_block']=$this->url->link('information/info_category_list', 'path=26_46');
		$data['by_country']=$this->url->link('information/info_category_list', 'path=26_33');
		$data['by_cooking']=$this->url->link('information/info_category_list', 'path=26_40');
		$data['by_meat']=$this->url->link('information/info_category_list',  'path=26_43');

		$data['text_block'] = $this->language->get('text_block');
		$data['text_country'] = $this->language->get('text_country');
		$data['text_cooking'] = $this->language->get('text_cooking');
		$data['text_meat'] = $this->language->get('text_meat');
// Product Order Category
...
$data['categories'] = array();
    ...
    $info_categories = $this->model_catalog_info_category->getInfoCategories(0);

    foreach ($info_categories as $info_category) {

        if ($info_category['top']) {
            // Level 2
            $info_children_data = array();

            $info_children = $this->model_catalog_info_category->getInfoCategories($info_category['category_id']);

            foreach ($info_children as $info_child) {

                // Level 3 sultan
                $info_children_data_3 = array();

                $info_children_3 = $this->model_catalog_info_category->getInfoCategories($info_child['category_id']);


                foreach ($info_children_3 as $info_child_3) {

                    $info_filter_data_3 = array(
                        'info_filter_category_id'  => $info_child_3['category_id'],
                        'info_filter_sub_category' => true
                    );

                    $info_children_data_3[] = array(
                        'name'  => $info_child_3['name'] ,
                        'href'  => $this->url->link('information/category_information', 'path=' . $child['category_id'] . '_' . $child_3['category_id'])
                    );
                }
                //end of level 3


                $info_filter_data = array(
                    'info_filter_category_id'  => $info_child['category_id'],
                    'info_filter_sub_category' => true
                );

                $info_children_data[] = array(
                    'name'  => $info_child['name'],
                    'href'  => $this->url->link('information/category_information', 'path=' . $info_category['category_id'] . '_' . $info_child['category_id']),
                    'grand_childs' => $info_children_data_3
            ...
            $data['info_categories'][] = array(
                ...
                'children' => $info_children_data,
                'column'   => $info_category['column'] ? $category['column'] : 1,
                'href'     => $this->url->link('information/category_information', 'path=' . $info_category['category_id'])
            );
        }
